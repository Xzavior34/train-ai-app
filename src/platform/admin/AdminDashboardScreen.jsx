...
import { Plus, Users, Layers, BookOpen, GraduationCap, Target, UserCheck, Mail, Flag, MoreHorizontal, AlertTriangle, ChevronRight, Star, CalendarClock } from "lucide-react";
import { useSupabaseQuery } from "../../lib/useSupabaseQuery.js";
import { fetchOrgDashboardStats, fetchTodaysTasks, fetchCohortProgressSummary, fetchStudentRiskList, fetchTopMentors, fetchUpcomingOrgSessions } from "../../lib/api/platform.js";

export function AdminDashboardScreen({ orgId, profileQuery, setScreen, orgSelector }) {
  ...
  const orgStats = statsQuery.data ? [
    ...
    { label: "Total mentors", value: statsQuery.data.mentors, icon: GraduationCap },
    { label: "Completion rate", value: `${statsQuery.data.completionRate}%`, icon: Target },
  ] : [];

  const todaysTasks = tasksQuery.data ? [
    { label: "Mentor applications to review", count: tasksQuery.data.mentorApplications, icon: UserCheck, tone: "primary", go: "people" },
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.6fr) minmax(0, 1fr)", gap: 24, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                  ...
                  <div key={s.name} className="ta-row ta-between" style={{ padding: "11px 12px", background: "var(--surface-3)", borderRadius: 12, border: "1px solid var(--border)" }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11.5, color: "var(--text-2)", marginTop: 2 }}>
                        {s.days === "—" ? "No recent activity on record" : `Inactive for ${s.days} day${s.days === 1 ? "" : "s"}`}
                      </div>
                    </div>
            ...
            <div className="ta-card">
              <div className="ta-row ta-between" style={{ paddingBottom: 12, borderBottom: "1px solid var(--border)" }}>
                <div>
                  <div className="ta-title">Top mentors</div>
                  <div className="ta-sub" style={{ marginTop: 2, fontSize: 12 }}>Highest-rated active mentors in your org</div>
                </div>
              ...
              <div className="ta-col ta-gap12 ta-mt16 anim-stagger">
                ...
                {!mentorsQuery.loading && (mentorsQuery.data || []).length === 0 && (
                  <div className="ta-empty">No active mentors yet.</div>
                )}
            ...
            <div className="ta-card">
              <div className="ta-row ta-between" style={{ paddingBottom: 12, borderBottom: "1px solid var(--border)" }}>
                <div>
                  ...
                  <div className="ta-sub" style={{ marginTop: 2, fontSize: 12 }}>Live & confirmed mentorship sessions</div>
                </div>

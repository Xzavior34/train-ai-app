    ...
    <div className="ta-fade">
      ...
      <div className="ta-content">
        ...
        <div className="ta-grid ta-grid-4">
          <StatCard stat={{ label: "Instructor rating", value: loading ? "—" : (mentor?.rating != null ? Number(mentor.rating).toFixed(1) : "0.0"), icon: Star }} />
          <StatCard stat={{ label: "Total sessions", value: loading ? "—" : (mentor?.total_sessions ?? 0), icon: Calendar }} />
          <StatCard stat={{ label: "Mentees helped", value: loading ? "—" : menteesHelped, icon: Users }} />
          <StatCard stat={{ label: "Earnings to date", value: loading ? "—" : `$${totalEarnings.toFixed(2)}`, icon: DollarSign }} />
        </div>
      </div>
    </div>
  );

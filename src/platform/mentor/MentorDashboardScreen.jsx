...
export function MentorDashboardScreen({ mentorId }) {
  ...
  return (
    <div className="ta-fade">
      <TopBar title="Instructor Overview" sub="Your upcoming mentorship sessions & earnings" />
      <div className="ta-content">
        ...
        <div className="ta-card ta-mt20">
          ...
          <div className="ta-col ta-gap10 ta-mt14">
            ...
            {!sessionsQuery.loading && upcomingSessions.length === 0 && <div className="ta-empty">No upcoming mentorship sessions.</div>}
            {upcomingSessions.map(s => (
              <div key={s.id} className="ta-row ta-between" style={{ padding: 12, background: "var(--surface-3)", borderRadius: 12 }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14 }}>{s.topic || s.title || "Mentorship Session"}</div>

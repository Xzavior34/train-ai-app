    ...
    <div>
      ...
      <div className="ta-grid ta-grid-2 ta-mt16">
        ...
        <select className="ta-input" value={role} onChange={(e) => setRole(e.target.value)}>
          {INVITE_ROLES.map((r) => <option key={r} value={r}>{r[0].toUpperCase() + r.slice(1)}</option>)}
        </select>
      </div>
  ...
  return (
    <div>
      ...
      <div className="ta-col ta-gap10 ta-mt20">
        <SummaryRow icon={Building2} label="Organization" done value={`${org?.name || "—"} — ${org?.resumed ? "resumed existing org" : "created"}`} />

...
import { Moon, ShieldCheck, Download, LogOut, ChevronRight, Sparkles, Trophy, Accessibility, Camera, AlertTriangle, Trash2, Clock, Smartphone, Gift, Bell } from "lucide-react";
...
import ReferralPanel from "../../components/common/ReferralPanel.jsx";
import { usePushNotifications } from "../hooks/usePushNotifications.js";

export function ProfileScreen({ user, dark, setDark, signOut, back, push, onSwitchToPlatform, credits, onBuyCredits, session, onAvatarUploaded, showToast }) {
  ...
  return (
    <div className="tai-fade-in">
      ...
      {session?.user?.id && (
        <div className="tai-card tai-mt12" style={{ cursor: "pointer" }} onClick={() => setShowReferral(true)}>
          <div className="tai-row tai-between">
            <div className="tai-row tai-gap10">
              <Gift size={18} color="var(--primary)" />
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 700 }}>Refer a friend</div>
                <div style={{ fontSize: 11.5, color: "var(--text-2)" }}>Share your link and track sign-ups</div>
              </div>
            </div>
            <ChevronRight size={16} color="var(--text-3)" />
          </div>
      ...
      <div className="tai-card tai-mt12">
        ...
        {onSwitchToPlatform && (
          <>
            <hr className="tai-divider" />
            <div className="tai-row tai-between" style={{ cursor: "pointer" }} onClick={onSwitchToPlatform}>
              <div className="tai-row tai-gap10">
                <ShieldCheck size={18} color="var(--primary)" />
                <span style={{ fontSize: 13.5, fontWeight: 600 }}>Switch to Platform Workspace</span>
              </div>
              <ChevronRight size={16} color="var(--text-3)" />
            </div>
          </>
      ...
      {showReferral && session?.user?.id && <ReferralPanel userId={session.user.id} onClose={() => setShowReferral(false)} />}

...
export function MentorsScreen({
  ...
  async function confirmBooking() {
    ...
    showToast("Mentorship session requested!");
  }
  ...
  return (
    <div className="tai-fade-in">
      <TopBar title="Mentors & 1-on-1 Sessions" sub="Book expert mentorship" />
      ...
      {requestingSession && sessionMentorChoice && (
        <div className="tai-card tai-mt12" style={{ borderColor: "var(--primary)" }}>
          ...
          {mentorAvailabilityQuery?.loading && <div style={{ fontSize: 12, color: "var(--text-2)", marginTop: 6 }}>Loading this mentor's availability...</div>}
          {!mentorAvailabilityQuery?.loading && !hasAvailability && (
            <div style={{ fontSize: 12, color: "var(--text-2)", marginTop: 6 }}>This mentor hasn't set recurring availability yet — your request will be sent as a tentative booking for tomorrow.</div>
          )}
          {hasAvailability && (
      ...
      <div className="tai-col tai-gap12 tai-mt16">
        {mentorsList.length === 0 && <div className="tai-empty">No active mentors available at this time.</div>}
        {mentorsList.map(m => {

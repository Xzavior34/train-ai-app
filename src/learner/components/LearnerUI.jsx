...
import {
  Home, BookOpen, Zap, Users, Settings, ArrowLeft, GraduationCap, Bookmark, Clock, CheckCircle2, Repeat, ChevronRight, Bell
} from "lucide-react";
...
export function TopBar({ title, sub, onBack, right, hideBell }) {
  const { unread, onOpen } = useContext(NotificationBellContext);
  return (
    <div className="tai-topbar">
      ...
      <div className="tai-row tai-gap8">
        {right}
        {!hideBell && onOpen && (
          <button className="tai-iconbtn" onClick={onOpen} aria-label="Notifications" style={{ position: "relative" }}>
            <Bell size={17} />
            {unread > 0 && <span className="anim-pulse-dot" style={{ position: "absolute", top: 6, right: 7, width: 7, height: 7, borderRadius: "50%", background: "var(--danger)" }} />}
          </button>
        )}
      </div>
    </div>
...
export function DesktopSidebar({ active, go, onProfile, profileActive, onOpenDashboardSwitcher, brandLogoUrl }) {
  return (
    <div className="tai-desktop-sidebar">
      ...
      <div className="tai-desktop-nav-footer">
        {onOpenDashboardSwitcher && (
          <div className="tai-desktop-nav-item" onClick={onOpenDashboardSwitcher}>
            <Repeat size={18} /><span>Switch Dashboard</span>
          </div>
        )}
...
export function CourseCard({ course, onClick, onEnroll, onToggleBookmark }) {
  return (
    <div className="tai-card" style={{ cursor: "pointer", position: "relative" }} onClick={onClick}>
      {onToggleBookmark && (
        <button
        ...
        >
          <Bookmark size={17} color={course.isBookmarked ? "var(--primary)" : "var(--text-3)"} fill={course.isBookmarked ? "var(--primary)" : "none"} />
        </button>
      )}
      <div className="tai-row tai-gap12">

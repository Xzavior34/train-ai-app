...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Authorization check - this function uses the SERVICE ROLE client
    // below, which bypasses RLS entirely, so this is the only boundary
    // standing between "moderate my own post" and "flip anyone's post to
    // approved/rejected by guessing its id." Previously missing entirely:
    // any authenticated caller could pass any contentId. Uses the
    // anon-key, RLS-respecting authClient for this check specifically so
    // it reflects the real access rules, not a hand-rolled copy of them.
    if (contentType === "post") {
      const { data: postRow, error: postErr } = await authClient
        .from("community_posts")
        .select("id, user_id")
        .eq("id", contentId)
        .maybeSingle();
      if (postErr || !postRow) {
        return jsonResponse({ error: "Post not found or not accessible" }, 404);
      }
      if (postRow.user_id !== userData.user.id) {
        return jsonResponse({ error: "Not authorized to moderate this post" }, 403);
      }
    }
